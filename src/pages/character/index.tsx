...
// Cada vez que se inicie la pag va estar en true con el loading y setLoading
const CharacterPage: React.FC = () => {
    const {id} = useParams()

    const [loading, setLoaging] = React.useState<boolean>(true);
    const [character, setCharacter] = React.useState< ICharacter | null>(null);
    

    React.useEffect(() =>{
        characters.getById({id}).then((r) => {setCharacter(r.data)
            setLoaging(false)
